...
namespace VRTK
{
    ...
    public class VRTK_DashTeleport : VRTK_HeightAdjustTeleport
    {
        ...
        [Tooltip("The cooldown between teleports in seconds.")]
        public float TeleportCooldown = 0.5f;
        [Tooltip("The reference to the slider to display the cooldown period.")]
        ...
        {
            EndTime = Time.deltaTime + TeleportCooldown;

            while (CurrentTime < EndTime)
            {
                CurrentTime += Time.deltaTime;
                float lerpValue = CurrentTime / TeleportCooldown;
                CooldownSlider.value = Mathf.Lerp(0.0f, 1.0f, lerpValue);
                yield return null;
            }
        }
